...
try:
    # Check if the full cloudstorage package exists. The stub part is in
    # runtime.
    cloudstorage = None
    import cloudstorage
...
class InputReader(json_util.JsonMixin):
    ...
    def next(self):
        ...
        raise NotImplementedError(
            "next() not implemented in %s" % self.__class__)
...
class AbstractDatastoreInputReader(InputReader):
    ...
    def split_input(cls, mapper_spec):
        ...
        if query_spec.ns is not None:
            ...
            # When number of ns is large, we can only split lexicographically
            # by ns.
            else:
    ...
    def _split_ns_by_scatter(cls,
        ...
        """
        if shard_count == 1:
            # With one shard we don't need to calculate any split points at
            # all.
        ...
        if len(k_ranges) < shard_count:
            # We need to have as many shards as it was requested. Add some
            # Nones.
    ...
    def validate(cls, mapper_spec):
        ...
        if cls.NAMESPACES_PARAM in params:
            raise BadReaderParamsError(
                "Multiple namespaces are no longer supported")
        if cls.FILTERS_PARAM in params:
            ...
            for f in filters:
                if not isinstance(f, (tuple, list)):
                    raise BadReaderParamsError(
                        "Filter should be a tuple or list: %s", f)
                if len(f) != 3:
                    raise BadReaderParamsError(
        ...
        if cls.FILTERS_PARAM in params:
            ...
            for f in filters:
                ...
                if not isinstance(f[0], basestring):
                    raise BadReaderParamsError(
                        "First element should be string: %s", f)
                if f[1] != "=":
                    raise BadReaderParamsError(
                        "Only equality filters are supported: %s", f)
...
class BlobstoreZipInputReader(InputReader):
    ...
    def next(self):
        ...
        if not self._zip:
            ...
            # Get a list of entries, reversed so we can pop entries off in
            # order
            self._entries = self._zip.infolist()[
    ...
    def _read(self, entry):
        ...
        if ctx:
            operation.counters.Increment(
                COUNTER_IO_READ_BYTES, len(content))(ctx)
            operation.counters.Increment(
...
class NamespaceInputReader(InputReader):
    ...
    def __iter__(self):
        while True:
            keys = self.ns_range.make_datastore_query().Get(
...
class LogInputReader(InputReader):
    ...
    # Semi-hidden parameters used only internally or for privileged
    # applications.
    ...
    def from_json(cls, json):
        ...
        if cls._OFFSET_PARAM in params:
            params[cls._OFFSET_PARAM] = base64.b64decode(
                params[cls._OFFSET_PARAM])
        return cls(**params)
    ...
    def to_json(self):
        ...
        if self._OFFSET_PARAM in params:
            params[self._OFFSET_PARAM] = base64.b64encode(
                params[self._OFFSET_PARAM])
        return params
    ...
    def split_input(cls, mapper_spec):
        ...
        # Create a LogInputReader for each shard, modulating the params as we
        # go.
        shards = []
...
class _GoogleCloudStorageInputReader(InputReader):
    ...
    def get_params(cls, mapper_spec, allowed_keys=None, allow_old=True):
        ...
        if (mapper_spec.params.get(cls.BUCKET_NAME_PARAM) is not None and
            ...
            params[cls.BUCKET_NAME_PARAM] = mapper_spec.params[
...
class _ReducerReader(_GoogleCloudStorageRecordInputReader):
                ...
                if combiner:
                    ...
                    for value in combiner_result:
                        if isinstance(value, operation.Operation):
                            ...
                            # With combiner the current values always come from
                            # the combiner.
                    ...
                    # check-point here would cause the current to_yield data to
                    # be lost.
                    if not to_yield:
                        yield ALLOW_CHECKPOINT
                else:
    ...
    def from_json(cls, json):
        ...
        result.current_values = _ReducerReader.decode_data(
            json["current_values"])
        return result
